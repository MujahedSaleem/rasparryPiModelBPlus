...
export class PatchesService {
    ...
    public connect(source: SourceComponent, sink: SinkComponent) {
        if (this.getConnectionsFor(source) && source.devicename != sink.parent)
    ...
    public get AllConnections(): any[] {
        return this.connections.map((data: Connection) => {
            return {
                sinkName: data.sink.parent, sinkPort: data.sink.name,
                sourceName: data.source.devicename, sourcePort: data.source.name
            }
        })
    }
